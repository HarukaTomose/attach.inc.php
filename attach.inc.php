...
define('PLUGIN_ATTACH_MAX_FILESIZE', (1024 * 1024 *5)); // default: 1MB
...
function attach_mime_content_type($filename, $displayname)
{
	...
	if (preg_match('/^(gif|jpg|jpeg|png|swf|mpo)$/i', $ext0)) {
	...
	function info($err)
	{
		...
		if ($this->age) {
			...
			if ($this->status['freeze']) {
				...
				$msg_thumb = '<input type="radio" name="pcmd" id="_p_attach_thumb" value="thumb" />' .
					'<label for="_p_attach_delete" >' .  $_attach_messages['msg_getthumb'] ;
				$msg_thumb .= '</label>';
				$msg_thumb .= '<input type="text" name="width" size="3" value="50%" /> x ';
				$msg_thumb .= '<input type="text" name="height" size="3" value="50%" /> <br />';
...
class ThumbFile
{
	...
	function ThumbFile($page, $file, $dstw, $dsth)
	{
		...
		$this->width    = $dstw;
		$this->height   = $dsth;
		$this->filename = CACHE_DIR . encode($this->page) . '_' . $dstw . 'x' . $dsth . '_thumb_' . $file;
		$this->exist    = file_exists($this->filename);
		$this->time     = $this->exist ? filemtime($this->filename) - LOCALZONE : 0;
	}
